...
public class RadialMenuScreen extends Screen {
    ...
    public RadialMenuScreen(List<Block> blocks, int slot, Block currentBlock, Block baseBlock) {
        ...
        this.blocks = blocks;
        this.blockStacks = new ArrayList<>(blocks.size());
        for (Block block : blocks) {
            this.blockStacks.add(new ItemStack(block));
        }
        this.slot = slot;
    ...
    private void renderTooltips(DrawContext context, int centerX, int centerY) {
        if (hoveredIndex != -1) {
            ...
            drawCenteredTooltip(context, List.of(block.getName()), centerX, centerY / 6);
            ...
            if (config.enableDevMode) {
                ...
                List<Text> debugTooltip = List.of(
