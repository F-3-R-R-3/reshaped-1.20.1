...
public class CircleTexture {
    ...
    /**
     * Clears cached circle textures so they can be regenerated after resource reload.
     */
    ...
    /**
     * Pre-generates radial menu slice textures for the given group sizes.
     * This mirrors RadialMenuScreen's ring geometry for selected and unselected slices.
     */
    public static void prewarmRadialSlices(
            Collection<Integer> groupSizes,
            float resolutionMultiplier,
            int unselectedColor,
            int selectedColor
    ) {
        final int radialRadius = 80;
        final int radialRingThickness = 20;
        final int halfThickness = radialRingThickness / 2;
        final int innerDiam = radialRadius - halfThickness;
        final int outerDiam = radialRadius + halfThickness;

        int resOuterUnselected = Math.round(outerDiam * resolutionMultiplier);
        int resInnerUnselected = Math.round(innerDiam * resolutionMultiplier);
        int resOuterSelected = Math.round((outerDiam + 6) * resolutionMultiplier);
        int resInnerSelected = Math.round((innerDiam - 4) * resolutionMultiplier);

        for (int groupSize : groupSizes) {
            if (groupSize <= 0) continue;

            float sectionWidth = 360f / groupSize;
            for (int slice = 0; slice < groupSize; slice++) {
    ...
    public static CachedCircle getOrCreateCircle(
        ...
        CachedCircle cached = CACHE.get(key);
        if (cached != null) {
            return cached;
        }
        ...
        CachedCircle newCached = new CachedCircle(textureId, size);
