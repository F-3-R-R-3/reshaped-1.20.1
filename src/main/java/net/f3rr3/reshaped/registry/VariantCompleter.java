        ...
        if (!hasSlab) {
            ...
            if (existingSlab != null) {
                if (!variants.contains(existingSlab)) {
                    ...
                    Reshaped.LOGGER.info("Adopted existing slab: " + Registries.BLOCK.getId(existingSlab) + " for " + baseId);
                }
            } else {
        ...
        if (!hasStair) {
            ...
            if (existingStairs != null) {
                if (!variants.contains(existingStairs)) {
                    ...
                    Reshaped.LOGGER.info("Adopted existing stairs: " + Registries.BLOCK.getId(existingStairs) + " for " + baseId);
                }
            } else {
                String cleanBase = baseId.getPath().replace("_planks", "").replace("_block", "");
